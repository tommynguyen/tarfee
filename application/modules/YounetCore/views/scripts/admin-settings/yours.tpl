...
<div>
        ...
        <tbody>
            <?php foreach ($this->modules as $key=>$item):?>
              ...
              <td><?php echo $item['name']; ?></td>
                        ...
                        <td>
                        <?php if(isset($item['params']) && $item['params'] !="" && $item['is_active'] == 1){if($latest_v > $current_v){echo '<a target="_blank" href="https://socialengine.younetco.com/ynuser-product-list">'.$action .'</a>';}else{echo $action; }}else{echo '<a href="'.$verify.'">Verify</a>';}?>
                        </td>
            </tr>
               <?php endif; ?>
          <?php endforeach; ?>
    ...
    <?php else: ?>
      <div class="tip">
        <span>
          <?php echo 'No plugins by YouNet Company were found on your site. Click <a href="http://socialengine.younetco.com" target="_blank">here</a> to view and purchase them.'; ?>
        </span>
      </div>
    <?php endif; ?>
    </div>
       ...
       <?php if (count($this->news['items'])>0):?>
           ...
           <ul class="news_ul">
                <?php foreach ($this->news['items'] as $n):?>  
                    <?php if(strpos($n['title'],'Protected:') === false):?>
                <li class="news_li">
                    <div class="news_title"><a href="<?php echo $n['link']?>"><?php echo $n['title']?></a><span><?php echo date('M-d-Y',strtotime($n['pubDate']))?></span></div>
                    <div class="clear"></div>
