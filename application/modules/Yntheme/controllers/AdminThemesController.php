 ...
 * @version    $Id: AdminThemesController.php 8385 2011-02-02 23:29:35Z john $
 * @author     Jung
 */
 ...
 * @license    http://www.socialengine.net/license/
 */
class Yntheme_AdminThemesController extends Core_Controller_Action_Admin
{
  ...
  public function exportThemeAction(){
  	...
  	$api =  Engine_Api::_()->getApi('Core','Yntheme');
	$tructure =  $api->exportStructure($theme);
	$redirect_url =  $this->getFrontController()->getRouter()->assemble(array('action'=>'dev'),'admin_default',false);
	return $this->_helper->redirector->setPrependBase(false)->gotoUrl($redirect_url);
  ...
  public function devAction(){
	...
	$this->view->navigation = $navigation = Engine_Api::_()->getApi('menus', 'core')
    	->getNavigation('yntheme_admin_main', array(), 'yntheme_admin_main_themes');
	...
	foreach($allThemes as $theme){
		$filename= APPLICATION_PATH . '/application/themes/'. $theme->name .'/manifest.php';
		$config =  include $filename ;
		if($config['package']['author'] == 'YouNet Company'){
			$themes[] = $theme;
		}
	}
	...
	$theme = $this->_getParam('theme','youface');
	$dir =  APPLICATION_PATH . '/application/themes/'. $theme . '/';
	...
	if($manifest['package']['author'] != 'YouNet Company'){
		return;
	}
	...
	$settingValue =  Engine_Api::_()->getApi('Settings','Core')->getSetting($settingKey,'default');
	
	$this->view->defaultSkin = $settingValue;
  }
  ...
  public function defaultSkinAction(){
	...
	// check theme and skin is exsist.
	
	if($skin != 'default'){
		$filename =  APPLICATION_PATH . '/application/themes/'. $theme . '/'. $skin . '/theme.css';
  	...
  	$settingKey = 'ynthemedefault.'. $theme;
	Engine_Api::_()->getApi('Settings','Core')->setSetting($settingKey,$skin);
	
	$url =  Zend_Controller_Front::getInstance()->getRouter()->assemble(array('module'=>'yntheme','controller'=>'themes','action'=>'index', 'theme'=>$theme),null,true);
  ...
  protected function _onActive($theme){
  	$filename =  APPLICATION_PATH . '/application/themes/'. $theme . '/database/install.sql';
	if(file_exists($filename) && is_readable($filename)){
		$this->_runScript($filename);
	}
  }
  
  protected function _onUnactive($theme){
  	$filename =  APPLICATION_PATH . '/application/themes/'. $theme . '/database/un-install.sql';
	if(file_exists($filename) && is_readable($filename)){
		$this->_runScript($filename);
	}
  }
  ...
  public function changeAction()
  {
	...
	$themeName = $this->_getParam('theme','default');
	
  	$from =  $this->_getParam('from','core');
	$options =  array('module'=>$from,'controller'=>'themes');
	
  	$redirect_url =  $this->getFrontController()->getRouter()->assemble($options,'admin_default',true);
	...
	$activeSelect =  $themeTable->select()->where('active=?',1);
	$currentActiveTheme =  $themeTable->fetchRow($activeSelect);
	
	$currentThemeName = 'default';
	if(is_object($currentActiveTheme)){
		$currentThemeName =  $currentActiveTheme->name;
	}
	
	// check if this themes is activated.
	if($theme->active){
		return $this->_helper->redirector->setPrependBase(false)->gotoUrl($redirect_url);
	}
	...
	//$this->_onUnactive($currentThemeName);
	//$this->_onActive($themeName);	
	
	$api =  Engine_Api::_()->getApi('Core','Yntheme');
	$api->importStructure($themeName,$currentThemeName);
	
	// backup old themes.
	
	$manifest =  $this->_getManifest($themeName);
	if($manifest['package']['author']=='YouNet Company' && isset($manifest['skins'])){
		$options =  array('module'=>'yntheme','controller'=>'themes','action'=>'index','theme'=>$themeName);
		$redirect_url =  $this->getFrontController()->getRouter()->assemble($options,'admin_default',true);
	}
		
		
  	// process new theme files.
  ...
  protected function _reloadThemes(){
  	// Get themes
