...
<div class="ynevent_discussions_thread_options">
  <?php echo $this->htmlLink(array('route' => 'event_extended', 'controller' => 'topic', 'action' => 'index', 'event_id' => $this->event->getIdentity(), 'tab' => $this -> tab), $this->translate('Back to Topics'), array(
  ...
  <?php elseif( !$this->canEdit ): ?>
    <?php if( $this->topic->closed ): ?>
      <div class="ynevent_discussions_thread_options_closed">
...
<script type="text/javascript">
  var quotePost = function(user, href, body) {
    if( $type(body) == 'element' ) {
      body = $(body).getParent('li').getElement('.ynevent_discussions_thread_body_raw').get('html').trim();
    }
  ...
  en4.core.runonce.add(function() {
    $$('.ynevent_discussions_thread_body').enableLinks();
  });
</script>
<ul class='ynevent_discussions_thread'>
  ...
  <li class="<?php echo $liClass ?>">
    <div class="ynevent_discussions_thread_author">
      <div class="ynevent_discussions_thread_author_name">
        <?php echo $this->htmlLink($user->getHref(), $user->getTitle()) ?>
      </div>
      <div class="ynevent_discussions_thread_photo">
        <?php echo $this->htmlLink($user->getHref(), $this->itemPhoto($user, 'thumb.icon')) ?>
      </div>
      <div class="ynevent_discussions_thread_author_rank">
        <?php
          if( $isOwner ) {
    ...
    <div class="ynevent_discussions_thread_info">
      <div class="ynevent_discussions_thread_details">
        <div class="ynevent_discussions_thread_details_options">
          <?php if( $this->form ): ?>
          ...
          <?php if( $post->user_id == $this->viewer()->getIdentity() ||
                    ...
                    $this->canAdminEdit ): 
                    $url = $this->url(array('controller' => 'post', 'action' => 'edit', 'post_id' => $post->getIdentity()),'event_extended', true);?>
            <a class="buttonlink icon_event_post_edit" href="javascript:;" onclick="checkOpenPopup('<?php echo $url?>');"><?php echo $this->translate('Edit')?></a>
        ...
        <div class="ynevent_discussions_thread_details_anchor">
          <a href="<?php echo $post->getHref() ?>">
            &nbsp;
          </a>
        </div>
        <div class="ynevent_discussions_thread_details_date">
          <?php echo $this->timestamp(strtotime($post->creation_date)) ?>
          <?php //echo $this->locale()->toDateTime(strtotime($post->creation_date)) ?>
        </div>
      </div>
      <div class="ynevent_discussions_thread_body">
        <?php echo nl2br($this->BBCode($post->body, array('link_no_preparse' => true))) ?>
      </div>
      <span class="ynevent_discussions_thread_body_raw" style="display: none;">
        <?php echo $post->body; ?>
      </span>
    </div>
  </li>
