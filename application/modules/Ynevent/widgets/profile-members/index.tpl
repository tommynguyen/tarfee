...
<script type="text/javascript">
    ...
    en4.core.runonce.add(function() {
        ...
        $('ynevent_members_search_input').addEvent('keypress', function(e) {
            if( e.key != 'enter' ) return;
            var filter = $('ynevent_members_filter').value;
        ...
        $$('.ynevent_members_filter select').each(function(el){
            el.removeEvents().addEvent('change', function(){                    
                var filter = el.value;
                var search = $('ynevent_members_search_input').value;
...
<?php if (!$this->waiting): ?>
    <div class="ynevent_members_info">
    	<?php if ($this->isOwner) : ?>
    	<a href="<?php echo $this->url(array('controller' => 'member', 'action' => 'export-csv', 'event_id'=> $this->event->getIdentity() ),'event_extended'); ?>" onclick="javascript:void(0);">
    		<?php echo $this->translate("Export CSV");?>
		</a>
		/
		<a href="<?php echo $this->url(array('controller' => 'member', 'action' => 'export-excel', 'event_id'=> $this->event->getIdentity() ),'event_extended'); ?>" onclick="javascript:void(0);">
    		<?php echo $this->translate("Export Excel");?>
		</a>
		<?php endif;?>
        <div class="ynevent_members_search">
            <input id="ynevent_members_search_input" type="text" value="<?php echo $this->translate('Search Guests'); ?>" onfocus="$(this).store('over', this.value);this.value = '';" onblur="this.value = $(this).retrieve('over');">
        </div>
        ...
        <div class="ynevent_members_total">
            <?php if ('' == $this->search): ?>
            	<?php if (($this->filter == '0' || $this->filter == '1' || $this->filter == '2') && ($this->members->getTotalItemCount() <= 0)) : ?>
    ...
    <div class="ynevent_members_info">
        <div class="ynevent_members_total">
            <?php echo $this->htmlLink('javascript:void(0);', 
           					...
           					$this->members->getTotalItemCount()),
           					$this->locale()->toNumber($this->members->getTotalItemCount())), 
