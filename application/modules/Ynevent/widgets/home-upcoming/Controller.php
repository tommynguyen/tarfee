...
class Ynevent_Widget_HomeUpcomingController extends Engine_Content_Widget_Abstract
{
	public function indexAction()
	{
		...
		if ($type == '2' || ($type == '0' && $viewer -> getIdentity()))
		{
			$eventMembership = Engine_Api::_() -> getDbtable('membership', 'ynevent');
		...
		$select -> where("`{$eventTableName}`.`endtime` > FROM_UNIXTIME(?)", time()) -> order("starttime ASC") -> group('repeat_group');
		// Make paginator
