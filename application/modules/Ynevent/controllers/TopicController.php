...
class Ynevent_TopicController extends Core_Controller_Action_Standard {
	...
	public function viewAction() {
		if (!$this->_helper->requireSubject('event_topic')->isValid())
			return;
		if (!$this->_helper->requireAuth()->setAuthParams()->isValid())
			return;
		...
		if ($viewer->getIdentity()) {
			$topicWatchesTable = Engine_Api::_()->getDbtable('topicWatches', 'ynevent');
		...
		if ($canPost && !$topic->closed) {
			$this->view->form = $form = new Ynevent_Form_Post_Create();
			$form->populate(array(
	...
	public function createAction() {
		...
		// Make form
		$this->view->form = $form = new Ynevent_Form_Topic_Create();
		...
		try {
			...
			$action = $activityApi->addActivity($viewer, $event, 'ynevent_topic_create', null, array('child_id' => $topic->getIdentity()));
			if( $action ) {
	...
	public function postAction() {
		...
		try {
			...
			// Activity
			$action = $activityApi->addActivity($viewer, $event, 'ynevent_topic_reply', null, array('child_id' => $topic->getIdentity()));
			if( $action ) {
	...
	public function renameAction() {
		...
		$this->view->form = $form = new Ynevent_Form_Topic_Rename();
	...
	public function deleteAction() {
		...
		$this->view->form = $form = new Ynevent_Form_Topic_Delete();
