...
<?php if( $this->paginator->getTotalItemCount() > 0 ): ?>
 ...
 <div class="tip">
    <span>
        <?php echo $this->translate('Add more %1$shere%2$s.', '<a  target = "_blank" href="'.$this->url(array('action' => 'add','type'=>0), 'ynbanmem_general').'">', '</a>'); ?>
        ...
        <table class='ynbanmem_table'>
            ...
            <tbody>
                ...
                <tr >
					...
					<td class='admin_table_email'><?php 
                        $user = Engine_Api::_()->getItem('user', $item['user'][0]['user_id']);
                        echo $user->email ?></td>
                    <td class=''><?php 
                    ...
                    <td class='admin_table_options'>
					 <?php if ($this->viewer->isAdmin() || Engine_Api::_()->authorization()->isAllowed('ynbanmem', $this->viewer, 'ban') ): // @todo change this to look up actual superadmin level ?>
                        <a  class='smoothbox'href='<?php 
							echo $this->url(array('action' => 'unban', 'id' => $item['banned_id'],'type'=>2));?>'>
                    ...
                    <a class='smoothbox' href='<?php 
                       ...
                       echo $this->url(array('action' => 'delete', 'id' => $item['user'][0]['user_id'],'type'=>2));?>'>
                       <?php echo $this->translate("delete") ?>
                       
                    </a>
                <?php endif;?>
                <?php if(($this->viewer->isAdmin() || Engine_Api::_()->authorization()->isAllowed('ynbanmem', $this->viewer, 'login')) && $item['user'][0]['level_id']!=1 ): // @todo change this to look up actual superadmin level ?>
                 </br>
...
<div class='buttons'>
    ...
    <button type='button' name="submit_button" value="unban" style="float: left;" onclick="multiUnban('<?php echo $this->url(array('action' => 'multi-unban','type'=>'email'));?>')"><?php echo $this->translate("Unban Selected") ?></button>
...
<div class="tip">
    <span>
        <?php echo $this->translate('Nobody has been banned yet.'); ?>

        <?php echo $this->translate('Add the email to banned list %1$shere%2$s.', '<a  href="'.$this->url(array('action' => 'add'), 'ynbanmem_general').'">', '</a>'); ?>
