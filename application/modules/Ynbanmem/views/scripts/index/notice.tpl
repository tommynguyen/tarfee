...
<?php if( count($this->paginator)): ?>
      ...
      $user = Engine_Api::_()->getItem('user', $conversation->user_id);
        ...
        <li<?php if( !$recipient->inbox_read ): ?> class='messages_list_new'<?php endif; ?> id="message_conversation_<?php echo $conversation->getIdentity() ?>">
          ...
          <div class="messages_list_from">
            <p class="messages_list_from_name">
              ...
              ?>
			   <?php switch($extra[0]['type'])
					{
						case 1:
							echo $this->translate('Notice');
						break;
						case 2:
							echo $this->translate('Warning');
						break;
						case 3:
							echo $this->translate('Infraction');
						break;
					}
				?>
			...
			<p> <?php echo $this->translate('Sent by ').$this->htmlLink($user->getHref(), $user->getTitle()) ?> </p>
             <p class="messages_list_from_date" title="reason" alt="reason">
              <?php echo $extra[0]['reason']; ?>
            </p>
          </div>
  ...
  <br />
  <?php if($flag):?>
  <button id="delete" style="float:right"><?php echo $this->translate('Delete Selected');?></button>
  <a href="javascript:void(0)" id="checkall">Check All</a>
 
  <?php else: ?>
  <p><?php echo $this->translate(array('You have 0 sent message total', 'You have 0 sent message total', $this->paginator->getTotalItemCount()), $this->locale()->toNumber($this->paginator->getTotalItemCount())) ?></p>
  <br />
...
<?php endif; ?>
 <script type="text/javascript">
  ...
  $('delete').addEvent('click', function(){
    ...
    var sb_url = '<?php echo $this->url(array('action'=>'delete-notice'), 'ynbanmem_general', true) ?>?place=outbox&message_ids='+selected_ids.join(',');
...
<?php else: ?>
  <p><?php echo $this->translate(array('You have %s sent message total', 'You have %s sent message total', $this->paginator->getTotalItemCount()), $this->locale()->toNumber($this->paginator->getTotalItemCount())) ?></p>
  <br />
