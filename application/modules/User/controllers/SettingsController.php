...
class User_SettingsController extends Core_Controller_Action_User {
	...
	public function privacyAction() {
		...
		$this -> view -> form = $form = new User_Form_Settings_Privacy( array('item' => $user, ));
	...
	public function passwordAction() {
		...
		$select = $userTable -> select() -> from($userTable, new Zend_Db_Expr('TRUE')) -> where('user_id = ?', $user -> getIdentity()) -> where('password = ?', new Zend_Db_Expr(sprintf('MD5(CONCAT(%s, %s, salt))', $db -> quote($salt), $db -> quote($form -> getValue('oldPassword'))))) -> limit(1);
		$valid = $select -> query() -> fetchColumn();
	...
	public function referralAction()
	{
		...
		$viewer = Engine_Api::_()->user()->getViewer();
		if( !$this->_helper->requireAuth()->setAuthParams('user_referral', null, 'allow_referral')->isValid() ) return;
		...
		if ($this -> getRequest() -> isPost()) 
		{
			if($totalCodes < $max_referral)
			{
				// Generate code
				do {
		        	$inviteCode = substr(md5(rand(0, 999) . time()), 10, 7);
		...
		{
			$used_codes[] = $usedCode -> code;
		}
		$this -> view -> used_codes = implode(', ', $used_codes);
		
		$this -> view -> max_referral = $max_referral;
		$this -> view -> totalCodes = count($usedCodes) + count($availableCodes);
