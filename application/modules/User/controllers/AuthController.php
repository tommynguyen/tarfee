...
class User_AuthController extends Core_Controller_Action_Standard
{
	...
	{   
		...
		// Form not valid
		if (!$form -> isValid($this -> getRequest() -> getPost()) && ($this -> getRequest()->getParam('level') !=5))
		{ 
	...
	public function logoutAction()
	{
		...
		$onlineTable = Engine_Api::_() -> getDbtable('online', 'user') -> delete(array('user_id = ?' => $viewer -> getIdentity(), ));
		...
		{
			Engine_Api::_() -> getDbtable('logins', 'user') -> update(array('active' => false, ), array('login_id = ?' => $_SESSION['login_id'], ));
		...
		if ($facebook && 'none' != $settings -> core_facebook_enable)
		{
			/*
			 ...
			 'next' => 'http://' . $_SERVER['HTTP_HOST'] . $this->view->url(array(),
			'default', true),
			 ));
			 */
			if (method_exists($facebook, 'getAccessToken') && ($access_token = $facebook -> getAccessToken()))
			{
				$doRedirect = false;
				// javascript will run to log them out of fb
	...
	public function forgotAction()
	{
		...
		$user = Engine_Api::_() -> getDbtable('users', 'user') -> fetchRow(array('email = ?' => $form -> getValue('email')));
		...
		try
		{
			...
			Engine_Api::_() -> getApi('mail', 'core') -> sendSystem($user, 'core_lostpassword', array(
				...
				'object_link' => $this -> _helper -> url -> url(array(
					'action' => 'reset',
					'code' => $code,
	...
	public function resetAction()
	{
		...
		// Check code
		$forgotTable = Engine_Api::_() -> getDbtable('forgot', 'user');
		$forgotSelect = $forgotTable -> select() -> where('user_id = ?', $user -> getIdentity()) -> where('code = ?', $code);
		...
		if (strtotime($forgotRow -> creation_date) < $min_creation_date)
		{
			// @todo The strtotime might not work exactly right
			return $this -> _helper -> redirector -> gotoRoute(array(), 'default', true);
		}
	...
	public function facebookAction()
	{
		...
		if ($facebook -> getUser())
		{
			...
			if (!$viewer -> getIdentity())
			{
				...
				if ($facebook_uid)
				{
					$user_id = $facebookTable -> select() -> from($facebookTable, 'user_id') -> where('facebook_uid = ?', $facebook_uid) -> query() -> fetchColumn();
				}
			...
			else
			{
				...
				$info = $facebookTable -> select() -> from($facebookTable) -> where('user_id = ?', $viewer -> getIdentity()) -> limit(1) -> query() -> fetch();
				if (empty($info))
				{
				...
				{
					//if( !empty($info['facebook_uid']) && $info['facebook_uid'] !=
					// $facebook->getUser() ) {
					// Incorrect user
					// Should we reconnect?
					//} else {
		...
		try
		{
			...
			if (isset($_SESSION['twitter_token2'], $_SESSION['twitter_secret2']))
			{
				...
				if (!$viewer -> getIdentity())
				{
					...
					$info = $twitterTable -> select() -> from($twitterTable) -> where('twitter_uid = ?', $accountInfo -> id) -> query() -> fetch();
			...
			if (isset($_SESSION['twitter_token'], $_SESSION['twitter_secret'], $_GET['oauth_verifier']))
			{
				...
				if ($viewer -> getIdentity())
				{
					$info = $twitterTable -> select() -> from($twitterTable) -> where('user_id = ?', $viewer -> getIdentity()) -> query() -> fetch();
