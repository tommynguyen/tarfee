...
<script type="text/javascript">
  // Populate data
  var maxRecipients = <?php echo (2 - count($this->sports))?>;
  var to = {
    id : false,
    type : false,
    guid : false,
  ...
  en4.core.runonce.add(function() {
    if( !isPopulated ) { // NOT POPULATED
      new Autocompleter2.Request.JSON('to', '<?php echo $this->url(array('action' => 'suggest'), 'user_sport', true) ?>', {
        'minLength': 1,
        'delay' : 250,
        'selectMode': 'pick',
        ...
        'injectChoice': function(token){
          if(token.type == 'user'){
            ...
            var choice = new Element('li', {
              'class': 'autocompleter-choices sport',
              'html': token.photo,
              'id':token.label
            });
            new Element('div', {
