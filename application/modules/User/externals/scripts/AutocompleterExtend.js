        ...
        doPushSpan: function(name, toID, newItem, hideLoc, list, url, token){
          ...
          switch (token.type) {
			...
			case 'ynmultilisting_listing':
				...
				html += '<p class="listing-owner">'+token.owner+'</p>';
				html += '<p class="listing-category">'+token.category+'</p>';
				html += "<a href='javascript:void(0);' onclick='this.parentNode.destroy();removeFromToValue(\""+toID+"\", \""+hideLoc+"\", \""+this.element.get('id')+"\");'>x</a>";
				myElement.addClass("listing_tag");
