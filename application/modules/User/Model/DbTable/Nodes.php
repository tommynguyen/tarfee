...
class User_Model_DbTable_Nodes extends Engine_Db_Table {
	...
	public function insertAfter(User_Model_Node$node, $data) {
		if($node -> pleft == 1) {
	...
	public function insertBefore(User_Model_Node$node, $data) {
		if($node -> pleft == 1) {
			throw new Exception("can not insert after root");
		}
	...
	public function appendChild(User_Model_Node$node, $data) {
	...
	public function addChild(User_Model_Node$node, $data) {
		if(!is_object($node)) {
			$node = $this -> getRoot();
		}
	...
	public function getDescendent($node, $include_own =true) {
		if(!$node instanceof User_Model_Node) {
