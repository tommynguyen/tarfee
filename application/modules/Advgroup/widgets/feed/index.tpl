...
<?php if( !empty($this->feedOnly) && empty($this->checkUpdate)): // Simple feed only for AJAX
  echo $this->activitygroupLoop($this->activity, array(
...
<?php if( $this->enableComposer ): ?>
  <div class="activity-post-container">
    ...
    <form method="post" action="<?php echo $this->url(array('action' => 'post'), 'group_general', true) ?>" class="activity" enctype="application/x-www-form-urlencoded" id="activity-form">
...
<?php echo $this->activitygroupLoop($this->activity, array(
  'action_id' => $this->action_id,
  'viewAllComments' => $this->viewAllComments,
  'viewAllLikes' => $this->viewAllLikes,
  'getUpdate' => $this->getUpdate,
