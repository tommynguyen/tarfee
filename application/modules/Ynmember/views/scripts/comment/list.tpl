...
<?php if( !$this->page ): ?>
<div class='comments' id="comments-<?php echo $this->subject->review_id;?>">
<?php endif; ?>
  <div class='comments_options'>
    ...
    <?php $formId = "comment-form-" . $this->subject->review_id;?>
      - <a href='javascript:void(0);' onclick="$('<?php echo $formId;?>').style.display = '';$('<?php echo $formId;?>').body.focus();"><?php echo $this->translate('Post Comment') ?></a>
    <?php endif; ?>

    <?php if( $this->viewer()->getIdentity() && $this->canComment ): ?>
      <?php if( $this->subject->likes()->isLike($this->viewer()) ): ?>
        - <a href="javascript:void(0);" onclick="ynmember.unlike_review('<?php echo $this->subject->getIdentity()?>','<?php echo $this->subject->getType()?>', '<?php echo $this->subject->getIdentity() ?>')"><?php echo $this->translate('Unlike This') ?></a>
      <?php else: ?>
        - <a href="javascript:void(0);" onclick="ynmember.like_review('<?php echo $this->subject->getIdentity()?>','<?php echo $this->subject->getType()?>', '<?php echo $this->subject->getIdentity() ?>')"><?php echo $this->translate('Like This') ?></a>
      <?php endif; ?>
    <?php endif; ?>
  </div>
  <ul>
  ...
  <script type="text/javascript">
    en4.core.runonce.add(function(){
      $($('<?php echo $formId;?>').body).autogrow();
      en4.core.comments.attachCreateComment($('<?php echo $formId;?>'));
    });
  </script>
  <?php 
  	if( isset($this->form) ) 
  		echo $this->form
  		->setAttribs(array('id' => 'comment-form-' . $this->subject->review_id, 'style' => 'display:none;'))
  		->setAction($this->url(array("controller" => 'comment', 'action' => 'create'),'ynmember_general'))
  		->render();
  ?>
  <?php if( !$this->page ): ?>
