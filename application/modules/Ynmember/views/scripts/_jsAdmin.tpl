...
<script type="text/javascript">
  function addRatingField(obj) {
		if($('add_rating_field').get('value') == "")
		{
			return;
		}
	    var value = $('add_rating_field').get('value');
	    var url = en4.core.baseUrl+'admin/ynmember/ajax-rating/add-rating-field';
	    new Request.JSON({
	        url: url,
	        method: 'post',
	        data: {
			...
			{
				var str = "<input class='rating_type_row_input' type='text' data_id='"+ responseJSON.id +"' value='"+responseJSON.title+"' />"
				+" <span data-id='"+ responseJSON.id +"' class='delete_rating_icon_button delete_rating_icon'>&nbsp&nbsp&nbsp&nbsp</span>";
				var html = new Element("div.rating_type_row", {
					'html': str,
					'id' : 'rating_type_row_' + responseJSON.id,
				});
				$('add_rating_field_wrapper').grab(html);
				$('add_rating_field').set('value','');	
				registerEvents();
			}
	    }).send();
	}	
	function editRatingField(obj){
		...
		var url = en4.core.baseUrl+'admin/ynmember/ajax-rating/edit-rating-field';
	    new Request.JSON({
	        url: url,
	        method: 'post',
	        data: {
	 ...
	 function deleteRatingField(obj) {
	 	var is_confirm = confirm("Do you want to delete this rating field?");
	 	if(is_confirm)
	 	{
		    var id = this.get('data-id');
		    var url = en4.core.baseUrl+'admin/ynmember/ajax-rating/delete-rating-field';
		    new Request.JSON({
		        url: url,
		        method: 'post',
		        data: {
		        ...
		        onSuccess: function(responseJSON) 
				{
					var str_id = 'rating_type_row_'+ id;
					$(str_id).setStyle('display','none');
				}
		    }).send();
	    }
	    else
     ...
     function openCloseRating(obj){
		if(this.hasClass('open_all_icon')) //action close
		{
			$$('.rating.open_all_icon').setStyle('display','none');
			$$('.rating.close_all_icon').setStyle('display','block');
			$('add_rating_field_wrapper').setStyle('display','none');
		}
		else //action open
		{
			$$('.rating.open_all_icon').setStyle('display','block');
			$$('.rating.close_all_icon').setStyle('display','none');
			$('add_rating_field_wrapper').setStyle('display','block');
		}
	}
	
	 function openCloseReview(obj){
		if(this.hasClass('open_all_icon')) //action close
		{
			$$('.review.open_all_icon').setStyle('display','none');
			$$('.review.close_all_icon').setStyle('display','block');
			$('add_review_field_wrapper').setStyle('display','none');
		}
		else //action open
		{
			$$('.review.open_all_icon').setStyle('display','block');
			$$('.review.close_all_icon').setStyle('display','none');
			$('add_review_field_wrapper').setStyle('display','block');
		}
	}
  ...
  var registerEvents = function() {
  	$$('.review.close_all_icon').removeEvents().addEvent('click', openCloseReview);
  	$$('.review.open_all_icon').removeEvents().addEvent('click', openCloseReview);
  	$$('.rating.close_all_icon').removeEvents().addEvent('click', openCloseRating);
  	$$('.rating.open_all_icon').removeEvents().addEvent('click', openCloseRating);
	$$('.rating_type_row_input').removeEvents().addEvent('keyup', editRatingField );
	 $('add_rating_field_button').removeEvents().addEvent('click', addRatingField);
	 $$('.delete_rating_icon_button').removeEvents().addEvent('click', deleteRatingField);
