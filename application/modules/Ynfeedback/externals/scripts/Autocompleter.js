		...
		if( !this.options.alwaysOpen ) {
                ...
                if (this.options.autocompleteType=='message') {
                  ...
                  if (token.friends){
                    ...
                    this.doAddValueToHidden(token.label, friend_ids, 'toValues', true, ' tag_friend', token.url, token.owner_name, token.owner_url);
                  ...
                  {
                  	this.doAddValueToHidden(choice.id, token.id, 'toValues', true, null, token.url, token.owner_name, token.owner_url);
                  }
                }
                
	},
        
        doAddValueToHidden: function (name, toID, hideLoc, newItem, list, url, owner_name, owner_url){
          ...
          if (this.checkSpanExists(name, toID)){
            ...
            this.doPushSpan(name, toID, newItem, hideLoc, list, url, owner_name, owner_url);
          }
        },

        doPushSpan: function(name, toID, newItem, hideLoc, list, url, owner_name, owner_url){
          // summary:
          //		This is called to create a new span item with the id.
          var myElement = new Element("span");
          if (newItem){
            myElement.id = "tospan_"+name+"_"+toID;
            myElement.innerHTML = "<a target='_blank' href="+url+">"+name+"</a>";
          	if(!(typeof owner_name === 'undefined')){
          		myElement.innerHTML += " - by" + "<a href=" + owner_url + ">" + owner_name + "</a>";
			 };
			myElement.innerHTML += " <a href='javascript:void(0);' onclick='this.parentNode.destroy();removeFromToValue(\""+toID+"\", \""+hideLoc+"\");'>x</a>"; 
          }
          else{
            myElement.id = "tospan_"+name+"_"+toID;
            myElement.innerHTML = "<a target='_blank' href="+url+">"+name+"</a>";
          ...
          if(!(typeof owner_name === 'undefined')){
			    ...
			    var newOption = new Option(name, toID);
