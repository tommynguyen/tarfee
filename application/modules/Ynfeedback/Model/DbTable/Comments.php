...
class Ynfeedback_Model_DbTable_Comments extends Engine_Db_Table 
{
	protected $_rowClass = 'Ynfeedback_Model_Comment';
	protected $_custom = false;

	public function __construct($config = array())
	{
		if( get_class($this) !== 'Ynfeedback_Model_DbTable_Comments' ) 
		{
			$this->_custom = true;
		}
	...
	public function getAllCommentsUsers(Core_Model_Item_Abstract $resource)
	{
		...
		{
			if( $data['poster_type'] == 'user' && $data['poster_id'] != '0')
			{
    ...
    public function deleteCommentsByIdea($idea_id) {
        $where = array();    
        $where[] = $this->getAdapter()->quoteInto('resource_id = ?', $idea_id);
        $where[] = $this->getAdapter()->quoteInto('resource_type = ?', 'ynfeedback_idea');
        $this->delete($where);
    }   
}