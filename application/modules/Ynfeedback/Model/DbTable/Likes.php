...
class Ynfeedback_Model_DbTable_Likes extends Engine_Db_Table
{
	protected $_rowClass = 'Ynfeedback_Model_Like';
	...
	public function addLike(Core_Model_Item_Abstract $resource, Core_Model_Item_Abstract $poster)
	{
		...
		else if (is_string($poster))
		{
			$row->poster_type = 'user';
			$row->poster_id = 0;
			$row->poster_name = $poster;
		}
		$row->save();
    ...
    public function deleteLikesByIdea($idea_id) {
        $where = array();    
