...
class Ynfeedback_Model_Poll extends Core_Model_Item_Abstract
{
  ...
  {
    $optionsTable = Engine_Api::_()->getDbTable('options', 'ynfeedback');
  ...
  {
    return Engine_Api::_()->getDbtable('options', 'ynfeedback')->fetchAll(array(
  ...
  {
    $table = Engine_Api::_()->getDbtable('pollVotes', 'ynfeedback');
    return (bool) $table
      ->select()
      ->from($table, 'COUNT(*)')
      ->where('poll_id = ?', $this->getIdentity())
  ...
  {
    $votesTable = Engine_Api::_()->getDbTable('pollVotes', 'ynfeedback');
    $optionsTable = Engine_Api::_()->getDbtable('options', 'ynfeedback');
  ...
  {
    // delete poll votes
    Engine_Api::_()->getDbtable('pollVotes', 'ynfeedback')->delete(array(
      'poll_id = ?' => $this->getIdentity(),
    ));

    // delete poll options
    Engine_Api::_()->getDbtable('options', 'ynfeedback')->delete(array(
      'poll_id = ?' => $this->getIdentity(),
    ));
