<?php
class Yncomment_View_Helper_FeedFluentDisLikeList extends Engine_View_Helper_FluentList {
     ...
     * @return string
     */
    public function feedFluentDisLikeList($items, $translate = false, $action) {
        ...
        if ($action->dislikes()->isDislike($viewer)) {
            if ($num == 1) {
                ...
                $num = ($num - 1);
                $url = $this->view->url(array('module' => 'yncomment', 'controller' => 'like', 'action' => 'activity-dislike', 'action_id' => $action->getIdentity(), 'call_status' => 'public', 'other' => 0, 'notIncludedId' => $viewer->getIdentity()), 'default', true);
            ...
            foreach ($items as $item) {
                ...
                if ($num == 1) {
                    ...
                    $num = $num - 1;
                    $url = $this->view->url(array('module' => 'yncomment', 'controller' => 'like', 'action' => 'activity-dislike', 'action_id' => $action->getIdentity(), 'call_status' => 'public', 'other' => 0, 'notIncludedId' => $item->getIdentity()), 'default', true);
                    $escapedURL = $this->view->string()->escapeJavascript($url);
                    $link = "onclick=Smoothbox.open('$escapedURL')";
                    if($num == 1) {
