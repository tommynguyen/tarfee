<?php
class Yncomment_View_Helper_FluentLikeList extends Engine_View_Helper_FluentList {
    ...
    public function fluentLikeList($items, $translate = false) {
        ...
        if (Engine_Api::_()->getDbtable('likes', 'core')->getLike($subject, $viewer)) {
            if ($num == 1) {
                ...
                $num = ($num - 1);
                $url = $this->view->url(array('module' => 'yncomment', 'controller' => 'like', 'action' => 'likelist', 'resource_type' => $subject->getType(), 'resource_id' => $subject->getIdentity(), 'call_status' => 'public', 'other' => 0, 'notIncludedId' => $viewer->getIdentity()), 'default', true);
            ...
            foreach ($items as $item) {
                ...
                if ($num == 1) {
                    ...
                    $num = $num - 1;
                     $url = $this->view->url(array('module' => 'yncomment', 'controller' => 'like', 'action' => 'likelist', 'resource_type' => $subject->getType(), 'resource_id' => $subject->getIdentity(), 'call_status' => 'public', 'other' => 0, 'notIncludedId' => $item->getIdentity()), 'default', true);
                    $escapedURL = $this->view->string()->escapeJavascript($url);
                    $link = "onclick=Smoothbox.open('$escapedURL')";
                    if($num == 1) {
                        if(Engine_Api::_()->getDbtable('likes', 'core')->getLike($subject, $viewer)) {
