...
document.addEvent('domready', function(){
	...
	var navs = $('ynblog_pagination').getElements('a');

	// create a basic slideshow
	navSlideShow = new SlideShow('ynblog_navigation-slideshow', {
		autoplay: true,
		delay: 5000,
	...
	if($$('.ynblog_newsalbums').length > 0) {
		var itemWidth =  parseInt($$('.ynblog_newsalbums')[0].getStyle('width'));
		...
		$('ynblog_navigation-slideshow').style.width = slideWidth + "px";
		console.log(itemWidth);
		if($$('.ynblog_newsalbums').length >= 4) {
			$$('.ynblog_newsalbums').each(function(item, index){
