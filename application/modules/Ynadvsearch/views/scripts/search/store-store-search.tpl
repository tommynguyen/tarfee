<script type='text/javascript'>
    ...
    window.addEvent('domready',function(){
        if ($('filter_form')) $('filter_form').set('action', '<?php echo $this->url(array("action"=>"store-store-search"), "ynadvsearch_search", true) ?>');
        if($('query'))
        {
    ...
    var loadContents = function(url)
    {
        ...
        var ajax_params = {};
        if (url == '') {
            url = en4.core.baseUrl + 'widget/index/name/ynadvsearch.store-result';
            ajax_params = params;
        }
...
<div id="ynadvsearch_result" >
    <div class='count_results ynadvsearch-clearfix'>
        ...
        <span class="label_results"><?php echo $this->htmlLink(array('route' => 'socialstore_general'), ucfirst($this->label_content), array());?></span>
    </div>
</div>
