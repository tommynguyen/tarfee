<script type='text/javascript'>
    ...
    window.addEvent('domready', function() {
        if ($('filter_form')) $('filter_form').set('action', '<?php echo $this->url(array('action' => 'jobposting-job-search'),'ynadvsearch_search', true);?>');
        loadContents('');
        if($('query'))
        {
        ...
        if($('job_title'))
        {
            $('job_title').value = '<?php echo $this -> query?>';
        }
    ...
    var loadContents = function(url)
    {
        ...
        var ajax_params = {};
        if (url == '') {
            url = en4.core.baseUrl + 'widget/index/name/ynjobposting.jobs-listing';
            ajax_params = params;
        }
...
<div id="ynadvsearch_result" style="display: none">
    <div class='count_results ynadvsearch-clearfix'>
        ...
        <span class="label_results"><?php echo $this->htmlLink(array('route' => 'ynjobposting_job'), $this->label_content, array());?></span>
    </div>
</div>
